...
const SettingThreadChangePermission = () => {
    const dispatch = useDispatch();
    const [showPermission, setShowPermission] = useState(false);
    const [check1, setCheck1] = useState(false)
    const [check2, setCheck2] = useState(false)
    const [whoCanEditThread, setWhoCanEditThread] = useState("")
    ...
    let { who_can_edit_thread } = useSelector(state => {
        ...
        if (!Thread) return {}
        return { who_can_edit_thread: Thread.who_can_edit_thread }
    ...
    const handleSubmitWhoCanEditThread = () => {
        ...
        dispatch({
            type: Action.API_UPDATE_THREAD_PERMISSION,
            data: {
                thread_id: thread_id,
                who_can_edit_thread: whoCanEditThread
            },
            setShowPermission
        })
    }
    useEffect(() => {
        setWhoCanEditThread(who_can_edit_thread)
        return () => {
        }
    }, [])
    const permission = () => {
                    ...
                    <View style={{
                        ...
                        <View style={{
                            ...
                            <Text style={{ fontSize: 16, fontWeight: "500", textAlign: "center" }}>
                                Quyền sửa tên, ảnh đại diện và thông tin khác
                            </Text>
                        </View>
                        ...
                        <View style={{
                            ...
                            <TouchableOpacity style={{
                                ...
                                onPress={() => {
                                    setCheck1(true)
                                    setCheck2(false)
                                    setWhoCanEditThread("only_leader")
                                }}
                            >
                                <Text style={{
                                    flex: 2,
                                ...
                                <View style={{}}>
                                    {
                                        check1 || whoCanEditThread === "only_leader"
                                            ?
    ...
    return (
        <View>
            ...
            >
                <Feather name='edit' size={25} color='#00A48D' />

                <Text style={{ paddingLeft: 10, fontWeight: "300", color: '#333', fontSize: 15 }}>
                    Quyền chỉnh sửa thông tin nhóm
                                </Text>
...
export default SettingThreadChangePermission
